...
const NewUserForm = () => {
    ...
    // username/password live validation
    useEffect(() => {
    ...
    // username/password input reset (after firing isSuccess)
    useEffect(() => {
        if (isSuccess) {
            setUsername('')
            setPassword('')
    ...
    // username/password controlled inputs (via event handlers)
    const onUsernameChanged = e => setUsername(e.target.value)
    const onPasswordChanged = e => setPassword(e.target.value)
    const onRolesChanged = e => {
        const values = Array.from(
            e.target.selectedOptions, //options originally sourced from user state (notionally set as "User")
            (option) => option.value
        )
        setRoles(values)
    }

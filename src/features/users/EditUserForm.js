...
const EditUserForm = ({ user }) => {
    ...
    // username/password input reset (after firing isSuccess / isDelSuccess)
    useEffect(() => {
    ...
    const onRolesChanged = e => {
        const values = Array.from(
            e.target.selectedOptions, //html options originally sourced from user state (drilled down user.roles)
            (option) => option.value
        )
        setRoles(values)
    }
